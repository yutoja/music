    ...
    <thead>
      ...
      <div class="jian">
        <div class="jh">
          ...
          <button @click="guzhu" v-text="xi == 1 ? '关注' : '取消关注'" v-if="!$store.state.user || $store.state.user.profile.userId != $route.query.id"></button>
...
export default {
  ...
  methods: {
    guzhu() {
      this.guzh(this.dat.userPoint.userId, this.xi)
      this.xi = this.aaaa && this.xi == 1 ? 0 : 1
      this.aaaa = false
    }
  },
  computed: {
  ...
  created() {
    ...
    this.$http(`/user/detail?uid=${this.id}&cookie=${co}`)
      .then(value => {
        this.dat = value.data
      })
      .catch(() => {
    ...
    this.$http(`/user/playlist?uid=${this.id}&cookie=${co}`).then(value => {
